...
class CatanGameState:
    def __init__(self, players: List[str], board: Dict[str, Any]):
        self.players = players
        self.board = board
        self.turn = 0
        self.resources = {player: {"wood": 0, "brick": 0, "sheep": 0, "wheat": 0, "ore": 0} for player in players}
...
class MemoryPlayer(Player):
    ...
    def __init__(self, color, name=None, llm=None):
        ...
        if llm is None:
            ...
            self.api_version = llm.api_version
            self.llm_name = llm.model

            # Initialize LLM through Langchain (Currently just Azure OpenAI)
            self.llm = AzureChatOpenAI(
                api_key=self.api_key,
                azure_endpoint=self.endpoint,
                azure_deployment=self.llm_name,
...
register_player("LLM")(MemoryPlayer)