...
FOO_RUN_COMMAND = f"catanatron-play --players=AB,{OUR_PLAYER} --num=5 --config-map=MINI  --config-vps-to-win=5"
...
class CreatorAgent():
    ...
    def create_langchain_react_graph(self):
        ...
        def analyzer_node(state: CreatorGraphState):
            ...
            tools = [list_local_files, read_local_file, read_performance_history, read_prompt, write_prompt, web_search_tool_call]
...
def read_performance_history(_: str = "") -> str:
    ...
    if performance_history_path.stat().st_size > 16_000:
