...
OUR_PLAYER = "R" # The name of the player that is being created
...
EVOLVE_COUNTER_MAX = 1
# Set winning points to 5 for quicker game
FOO_RUN_COMMAND = f"catanatron-play --players=AB,{OUR_PLAYER} --num=1 --config-vps-to-win=5"
...
class CreatorAgent():
    ...
    def create_langchain_react_graph(self):
        ...
        def analyzer_node(state: CreatorGraphState):
            ...
            messages = [SystemMessage(content=summary_message)] + state_msgs
            temp_tools = []
            llm_with_tools = self.llm.bind_tools(temp_tools)
            print("Starting Invoke")
            summarizer_resp = llm_with_tools.invoke(messages)
            print("Finished Invoke")
            summarizer_response = HumanMessage(content=summarizer_resp.content)
