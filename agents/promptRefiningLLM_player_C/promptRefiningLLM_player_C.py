...
class PromptRefiningLLMPlayer_C(Player):
    ...
    def __init__(self, color, name=None, llm=None):
        ...
        if PromptRefiningLLMPlayer_C.run_dir is None:
            if "CATAN_CURRENT_RUN_DIR_C" in os.environ:
                # Use the directory passed from the creator agent
                PromptRefiningLLMPlayer_C.run_dir = os.environ["CATAN_CURRENT_RUN_DIR_C"]
            else:
                ...
                try:
                    with open(os.path.join(runs_dir, "current_run.txt"), "r") as f:
                        PromptRefiningLLMPlayer_C.run_dir = f.read().strip()
                except:
                    # Fallback to creating a new run directory
                    run_id = datetime.now().strftime("run_%Y%m%d_%H%M%S")
                    PromptRefiningLLMPlayer_C.run_dir = os.path.join(runs_dir, run_id)
                    os.makedirs(PromptRefiningLLMPlayer_C.run_dir, exist_ok=True)
    ...
    def _get_llm_decision(self, game_state_text: str, num_actions: int) -> Optional[int]:
        ...
        try:
            ...
            game_run_dirs = [d for d in pathlib.Path(PromptRefiningLLMPlayer_C.run_dir).glob("game_*") if d.is_dir()]
            if game_run_dirs:
                ...
                log_path = pathlib.Path(PromptRefiningLLMPlayer_C.run_dir) / f"llm_log_{self.llm_name}.txt"
