...
FOO_TARGET_FILENAME = "promptRefiningLLM_player_G.py"
    ...
    def create_langchain_react_graph(self):
        ...
        def analyzer_node(state: CreatorGraphState):
            ...
            sys_msg = SystemMessage(content =
                (
                    f"""
                    You are in charge of creating the prompt for the Catan Player promptRefiningLLM_player_G in {FOO_TARGET_FILENAME}. 
                    
                    YOUR PRIMARY GOAL: Create a prompt that helps our promptRefiningLLM_player_G win against its opponent AlphaBetaPlayer.
...
def read_performance_history(_: str = "") -> str:
    ...
    if performance_history_path.stat().st_size > 64_000:
        return "Performance history file is too large (>64 KB). Consider truncating or summarizing it."
...
def run_gamefoo(_: str = "") -> str:
    ...
    os.environ["CATAN_CURRENT_RUN_DIR_G"] = str(CreatorAgent.run_dir)
