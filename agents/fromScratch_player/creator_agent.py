...
FOO_TARGET_FILE = Path(__file__).parent / "foo_player.py"    # absolute path
FOO_MAX_BYTES   = 64_000                                     # context-friendly cap
FOO_RUN_COMMAND = "catanatron-play --players=R,FOO --num=10 --output=data/ --json"
...
class CreatorAgent():
    ...
    def __init__(self):
        ...
        shutil.copy2(                           # ↩ copy with metadata
            (Path(__file__).parent.parent / "foo_player.py").resolve(),  # ../foo_player.py
            (Path(__file__).parent / "foo_player.py").resolve()          # ./foo_player.py
        )
    ...
    def run_react_graph(self):
        prompt = (
            """
            You are in charge of creating the code for a Catan Player in foo_player.py. 
            ...
            - read_foo: Read the content of foo_player.py.
            - write_foo: Write the content of foo_player.py. (Make sure to keep imports)
            - run_testfoo: Test the content of foo_player.py in a game.
        ...
        try:
            ...
            shutil.copy2(                           
                (Path(__file__).parent / "foo_player.py").resolve(),
                (Path(CreatorAgent.run_dir) / "foo_player.py")
            )
...
def read_foo(_: str = "") -> str:
    """Return the UTF-8 content of foo_player.py (≤64 kB)."""
...
def write_foo(new_text: str) -> str:
    """Overwrite foo_player.py with new_text (UTF-8)."""
    ...
    return "foo_player.py updated successfully"

def run_testfoo(_: str = "") -> str:
    """Run one Catanatron match (R vs FOO) and return raw CLI output."""
